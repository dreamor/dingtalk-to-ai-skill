...
import {
  formatError,  getCLIInstallSuggestion,  formatRateLimitMessage,  formatBusyMessage,  } from './errorFormatter';
...
export class GatewayServer {
  ...
  constructor(dingtalkService: DingtalkService,  deps: GatewayDeps) {
    ...
    this.retrySender = new RetrySender({
      maxRetries: 5,  baseDelay: 5000,  maxDelay: 300000,  checkInterval: 10000,  });
    ...
    this.commandHandler = new CommandHandler({
      sessionManager: deps.sessionManager,  messageQueue: deps.messageQueue,  });
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/health',  (_req: Request,  res: Response) => {
      res.json({
        status: 'ok',  timestamp: new Date().toISOString(),  mode: 'stream',  });
    });
    ...
    this.app.post('/api/test',  async (req: Request,  res: Response) => {
      try {
        const result = await this.processMessage({
          msg: req.body.msg || '',  userId: 'test-user',  userName: '测试用户',  });
        res.json(result);
      } catch (error) {
        res.status(500).json({
    ...
    this.app.get('/api/queue',  (_req: Request,  res: Response) => {
      res.json({
        success: true,  data: this.messageQueue.getStatus(),  });
    });

    // 检查 AI Provider 状态
    this.app.get('/api/status',  async (_req: Request,  res: Response) => {
      const [opencodeAvailable,  claudeAvailable] = await Promise.all([
        this.openCodeExecutor.isAvailable(),  this.claudeCodeExecutor.isAvailable(),  ]);
      const queueStatus = this.messageQueue.getStatus();
      const retryQueueStats = this.retrySender.getStats();
      const rateLimitStatus = {
        maxTokensPerUser: this.rateLimiter.getMaxTokens(),  currentUsers: this.rateLimiter.getUserCount(),  };
      const concurrencyStatus = {
        maxPerUser: this.concurrencyController.getMaxSlotsPerUser(),  maxGlobal: this.concurrencyController.getMaxGlobalSlots(),  availablePerUser: this.concurrencyController.getAvailableSlots('testUser'),  availableGlobal: this.concurrencyController.getAvailableGlobalSlots(),  };

      res.json({
        success: true,  data: {
          aiProvider: config.aiProvider,  opencode: {
            available: opencodeAvailable,  command: config.ai.command,  timeout: config.ai.timeout,  maxRetries: config.ai.maxRetries,  },  claude: {
            available: claudeAvailable,  command: config.claude.command,  timeout: config.claude.timeout,  maxRetries: config.claude.maxRetries,  },  messageQueue: {
            pending: queueStatus.queued,  processing: queueStatus.processing,  byPriority: queueStatus.byPriority,  },  retryQueue: retryQueueStats,  rateLimit: rateLimitStatus,  concurrency: concurrencyStatus,  },  });
    });
    ...
    this.app.post('/api/card/send',  async (req: Request,  res: Response) => {
      try {
        ...
        if (!conversationId || !title || !content) {
          res.status(400).json({
            success: false,  message: '缺少必要参数：conversationId,  title,  content',  });
          return;
        }

        if (!this.streamService) {
          res.status(503).json({
            success: false,  message: 'Stream 服务未连接',  });
          return;
        }

        const cardData = CardBuilder.createMarkdownCard({
          title,  content,  buttons,  imageUrl,  });

        const sent = await this.streamService.sendCardMessage(conversationId,  cardData);

        res.json({
          success: sent,  message: sent ? '卡片发送成功' : '卡片发送失败',  });
      } catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
        console.error('[Gateway] 发送卡片失败:',  msg);
        res.status(500).json({
          success: false,  message: `发送卡片失败: ${msg}`,  });
      }
    });
    ...
    this.app.get('/api/memory/entries',  (req: Request,  res: Response) => {
      ...
      const filter: { category?: MemoryCategory; source?: MemorySource; query?: string; limit?: number; offset?: number } = {};
    ...
    this.app.post('/api/memory/entries',  (req: Request,  res: Response) => {
      ...
      try {
        const entry = this.memoryManager.createMemory({
          key,  value,  category,  source: source ?? 'manual',  relevanceScore,  });
    ...
    this.app.patch('/api/memory/entries/:id',  (req: Request,  res: Response) => {
      ...
      const updated = this.memoryManager.updateMemory(req.params.id,  {
        key,  value,  category,  source,  relevanceScore,  });
      if (updated) {
  ...
  async processMessage(
    request: GatewayRequest,  useQueue: boolean = false
    ...
    if (useQueue) {
      ...
      try {
        const userMessage: UserMessage = {
          id: generateMessageId(),  type: 'user',  conversationId: '',  userId,  username: userName,  content: msg,  metadata: {
            timestamp: Date.now(),  source: 'dingtalk',  },  };

        this.messageQueue.enqueue(userMessage,  'normal');

        return {
          success: true,  message: '消息已接收，正在处理中',  data: {
            messageId: userMessage.id,  },  };
      } catch (error) {
        console.error('[Gateway] 入队消息失败:',  error);
        return {
          success: false,  message: '消息接收失败',  };
      }
    } else {
  ...
  async handleStreamMessage(msg: string,  userId: string,  userName: string): Promise<void> {
        ...
        if (result.success && result.data?.result) {
          ...
          try {
            ...
            await this.dingtalkService.sendTextMessage(
              accessToken,  '📬 您的消息已收到，回复正在发送中，请稍候...'
            );
            return;
          }
        } else {
    ...
    try {
      ...
      await this.dingtalkService.sendTextMessage(
        accessToken,  '⚠️ 消息处理遇到问题，系统将自动重试，请稍候查看回复。'
      );
    } catch (_sendError) {
      ...
      .finally(() => {
        if (this.consumerRunning) {
          this.consumerTimer = setTimeout(
            () => this.consumeLoop(),  config.messageQueue.pollInterval
          );
        }
      });
  }
    ...
    const processPromises = queuedMessages.map(async queuedMsg => {
      ...
      try {
        ...
        await this.processMessageInternal({
          msg: message.content,  userId: message.userId,  userName: message.username || '用户',  });
  ...
  private async processMessageInternal(request: GatewayRequest): Promise<GatewayResponse> {
    ...
    if (parsedCommand) {
      ...
      try {
        ...
        return {
          success: true,  message: '命令处理完成',  data: { result: response },  };
      } catch (error) {
        return {
          success: false,  message: error instanceof Error ? error.message : '命令处理失败',  };
      }
    }
    ...
    if (this.deduplicator.isDuplicate(msg,  userId)) {
      ...
      return {
        success: false,  message: '消息已处理，请勿重复发送',  };
    }
    ...
    if (!rateLimitResult.allowed) {
      ...
      return {
        success: false,  message: formatRateLimitMessage(rateLimitResult.remaining),  };
    }
    ...
    try {
      ...
      return {
        success: false,  message: '会话创建失败，请稍后重试',  };
    }
    ...
    try {
      ...
      return {
        success: false,  message: formatBusyMessage(),  };
    }

    try {
      // 6. 创建用户消息对象
      const userMessage: UserMessage = {
        id: generateMessageId(),  type: 'user',  conversationId: session.conversationId,  userId,  username: userName,  content: msg,  metadata: {
          timestamp: Date.now(),  source: 'dingtalk',  },  };
      ...
      const opencodeContext: MessageContext = {
        userId,  userName,  conversationId: session.conversationId,  history,  memoryContext: memoryContext || undefined,  };
      ...
      const aiMessage: AIMessage = {
        id: generateMessageId(),  type: 'ai',  conversationId: session.conversationId,  userId,  content: responseContent,  metadata: {
          timestamp: Date.now(),  source: 'ai',  },  };
      ...
      return {
        success: result.success,  message: result.success ? '处理成功' : '处理失败',  data: {
          result: responseContent,  conversationId: session.conversationId,  executionTime: totalTime,  messageId,  },  };
    } finally {
      // 14. 释放并发槽位
